...
Runs, in one command:
  0a. rebuild mcq_rewrites.json (via build_rewrites_v2.py, honoring EXCLUDED_IDS) + promote
...
open variants.
  ...
  python evaluation/scripts/regen_mcq_tier01.py --provider ollama
  python evaluation/scripts/regen_mcq_tier01.py --provider ollama --skip-promote --models "1.5b=qwen2.5:1.5b"
"""
...
def promote(root):
    ...
    repo = here.parent
    print(">> rebuilding mcq_rewrites.json (build_rewrites_v2.py) ...")
    subprocess.run([sys.executable, str(here / "build_rewrites_v2.py")], cwd=repo, check=True)
...
def run(args):
    ...
    if not args.skip_promote and not args.rejudge_open and not args.report_only:
        promote(root)
    active = set(json.loads((Path(args.root).parent / "mcq_rewrites.json").read_text(encoding="utf-8")))
    ...
    def answer_cell(model_dir, ollama, ch, cond):
        ...
        if args.update_scores:
            cell_dir = root / f"luke{ch}" / model_dir / cond
    ...
    def rejudge_cell(model_dir, _ollama, ch, cond):
        ...
        qf = pick(src, "qa_target_pseudonymized.json", "qa_target_decanonicalized.json")
        pf = pick(src, "passage_target_pseudonymized.txt", "passage_target_decanonicalized.txt")
        scores_path = root / f"luke{ch}" / model_dir / cond / "scores_target_window3_v2.json"
