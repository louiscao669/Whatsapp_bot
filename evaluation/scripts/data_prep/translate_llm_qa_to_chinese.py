...
def normalize_item(item: dict, index: int) -> dict:
    ...
    # An all-formats record carries a general/open ``question`` at the top
    # level and an option-aligned ``mcq_stem`` inside its MCQ variant.  For an
    # MCQ the stem must win; otherwise choices written to answer "who/what"
    # can be paired with the open question asking "how/why".  ``Q`` remains
    # first because already-compact or translated records use it directly.
    if q_type == "mcq":
        question = (
            item.get("Q")
    ...
    for field in (
        "passage_id",
        "passage_reference",
        # The per-item verse ("18:30"). Dropping it left the answer stage with
        # only the whole-passage reference, so --answer-verse-window widened to
        # the entire passage on single-chapter texts and mistargeted entirely on
        # cross-chapter ones.
        "reference",
        "passage_text",
        "audio_url",
...
def translate_batch(
    ...
    """Translate one batch of QA items.
    ...
    prompt = {
        "task": (
            ...
            + (
                ...
                "yourself, do not leave them in the source script, and do not "
                "wrap them in braces or any other markup. The passage the "
                "reader sees uses these exact spellings; a different rendering "
                "makes the question refer to someone who does not appear in it."
                if glossary
                else ""
            )
        ),
    ...
    # Responses API supports temperature but not seed (a chat.completions
    # parameter). seed is accepted for interface symmetry and ignored.
    sampling: dict = {}
    ...
    for batch in batched(items, batch_size):
        ...
        for attempt in range(retries + 1):
            try:
                output.extend(
                    translate_batch(
                        client, model, batch, target_language, temperature, seed,
                        glossary,
                    )
                )
                last_error = None
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=0.0,
        help=(
            "Sampling temperature. Defaults to 0 for reproducibility. Pass 1.0 "
            "to reproduce pre-2026-08-03 runs."
        ),
    )
    parser.add_argument(
        "--seed",
        type=int,
        help="Ignored by the Responses API; kept for interface symmetry.",
    )
