...
def bank_path_for_chapter(bank_dir: Path, chapter: Any) -> Path:
    """Per-chapter bank path.

    Luke cells are addressed by integer chapter, so the bank is
    "luke3_awkward_style_bank.json". A named passage dir (tier1/t1_judg9) has
    no such bank; the path is still returned so the caller's existence check
    falls through to --fallback-bank.
    """
    if isinstance(chapter, int) or str(chapter).isdigit():
        return bank_dir / f"luke{chapter}_awkward_style_bank.json"
    stem = str(chapter).replace("/", "_")
    return bank_dir / f"{stem}_awkward_style_bank.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--bank-dir",
        type=Path,
        default=Path("evaluation/datasets/perturbations/awkward_style"),
        help="Directory containing lukeN_awkward_style_bank.json files.",
    )
    parser.add_argument(
        "--fallback-bank",
        type=Path,
        default=Path("evaluation/datasets/perturbations/awkward_style_bank.json"),
        help="Fallback global bank if chapter-specific bank is missing.",
    )
    parser.add_argument(
        "--require-bank",
        action="store_true",
        help="Fail instead of falling back to the global bank.",
    )
    ...
    for chapter in targets:
        ...
        if not bank_path.exists():
            if args.require_bank:
                # The fallback is a global bank built from Luke. Substituting it
                # for a passage-specific one produces a grid that looks correct
                # but perturbs Judges with Luke vocabulary, which the answer
                # model can detect as alien text rather than degraded meaning.
                # Under --require-bank that is an error, not a fallback.
                print(
                    f"error: no passage-specific awkward bank for {chapter}: {bank_path}\n"
                    f"       run generate_chapter_awkward_style_banks.py first, "
                    f"or drop --require-bank to use {args.fallback_bank}",
                    file=sys.stderr,
                )
                return 1
            if args.fallback_bank and args.fallback_bank.exists():
                # Previously silent. A fallback changes what the experiment
                # measures, so it is now always announced.
                print(
                    f"warning: no bank for {chapter} ({bank_path}); "
                    f"falling back to global bank {args.fallback_bank}",
                    file=sys.stderr,
                )
                bank_path = args.fallback_bank
            else:
                print(f"warning: bank missing for {chapter}: {bank_path}", file=sys.stderr)
                continue
