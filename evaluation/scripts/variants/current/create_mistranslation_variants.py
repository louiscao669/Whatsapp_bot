...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--require-bank",
        action="store_true",
        help="Fail instead of falling back to the built-in default bank.",
    )
...
def main() -> int:
    ...
    for chapter in targets:
        ...
        if bank_source == "built_in_default" and args.require_bank:
            # The built-in bank was written for Luke. Falling back to it makes a
            # grid that looks complete but perturbs this passage with another
            # text's vocabulary, so under --require-bank it is an error.
            print(
                f"error: no passage-specific mistranslation bank for {chapter} "
                f"({chapter_bank_path(source_model_dir, args.chapter_bank_name)})\n"
                f"       run generate_mistranslation_banks.py first, "
                f"or drop --require-bank to use the built-in default",
                file=sys.stderr,
            )
            return 1

        copy_shared_files(source_model_dir / "_shared", output_dir / "_shared")
        # Says the actual cell name; it used to hardcode "luke{chapter}", which
        # printed "luke tier1/t1_judg9" once passage dirs were allowed.
        print(f"using mistranslation bank for {chapter}: {bank_source}")
