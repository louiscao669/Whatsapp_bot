...
def build_prompt(chapter: Any, passage: str, max_items: int) -> str:
    # Quoted via json.dumps: the Luke cells pass an int, the Tier 1 cells a
    # name like "tier1/t1_judg9", and an unquoted name is invalid JSON.
    chapter_field = json.dumps(chapter, ensure_ascii=False)
    return f"""
You are creating a controlled mistranslation substitution bank for an MQM
Accuracy > Mistranslation experiment.

Task:
- Read the Chinese passage below.
...
{{
  ...
  "language": "zh",
  "description": "passage-specific LLM-assisted MQM mistranslation bank",
  "replacements": [
    {{
      "source": "牧羊人",
      "target": "渔夫",
...
def validate_bank(data: dict, *, chapter: Any, passage: str, model: str) -> dict:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "--chapters, which can only address the integer-keyed Luke cells."
        ),
    )
    parser.add_argument(
        "--output-name",
...
def main() -> int:
    ...
    for chapter in targets:
        source_model_dir = args.root / str(chapter) / args.source_model_dir
        ...
        try:
            ...
            print(f"warning: failed {chapter}: {exc}", file=sys.stderr)
            failures.append(str(chapter))

    if failures:
        # Previously this returned 0 even when every passage failed. Under a
        # `set -e` campaign that let the run continue to the variant step, which
        # falls back to the built-in Luke bank -- producing a complete-looking
        # grid silently built on the wrong vocabulary. Fail loudly instead.
        print(
            f"\nerror: {len(failures)} bank(s) not generated: {', '.join(failures)}",
            file=sys.stderr,
        )
        return 1
