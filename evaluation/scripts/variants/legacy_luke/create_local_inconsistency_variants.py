#!/usr/bin/env python3
"""Create question-local style inconsistency variants.
...
Style inconsistency is generated by LLM sentence/clause rewriting directly on
the Chinese local passage. Name inconsistency remains available only when
explicitly requested with --types name.
"""
...
from evaluation.scripts.variants.current.create_inconsistency_variants import (
    DEFAULT_RATES,
    DEFAULT_STYLE_MODEL,
    DEFAULT_TYPES,
    InconsistencyError,
    apply_inconsistency,
    apply_style_inconsistency_llm,
...
DEFAULT_BANK_DIR = Path("evaluation/datasets/perturbations/local_inconsistency")
...
def load_chapter_bank(chapter: int, bank_dir: Path) -> dict[str, list[str]] | None:
    """Load the per-chapter, identity-preserving name-variant bank.

    Returns a {placeholder: [variants]} dict (as consumed by apply_inconsistency
    for the "name" type), or None if no bank file exists for the chapter.
    """
    path = bank_dir / f"inconsistency_bank_luke{chapter}.json"
    if not path.exists():
        return None
    data = json.loads(path.read_text(encoding="utf-8"))
    variants = data.get("variants", data)
    return {str(k): list(v) for k, v in variants.items()}
...
def mutate_qa_local_passages(
    ...
    *,
    ...
    # This module defines its own index_passage_verses (bare verse-number keys),
    # not the chapter-aware one in agents/generate_chinese_answers.py.
    ...
    for index, item in enumerate(items, start=1):
        ...
        if cache_key not in cache:
            ...
            local_seed = stable_seed(seed, inconsistency_type, rate, key, reference)
            if inconsistency_type == "style":
                if style_client is None:
                    raise InconsistencyError(
                        "style_client is required for local style inconsistency variants."
                    )
                mutated, local_meta = apply_style_inconsistency_llm(
                    local_passage,
                    rate=rate,
                    seed=local_seed,
                    client=style_client,
        ...
        item["local_inconsistency"] = {
            ...
            "mechanism": local_meta.get("mechanism", "replacement"),
            "requested_rate": rate,
    ...
    return container, {
        "type": inconsistency_type,
        "mechanism": (
            "llm_clause_rewrite" if inconsistency_type == "style" else "replacement"
        ),
        "requested_rate": rate,
        "actual_affected_rate": actual_rate,
        "seed": seed,
        "model": style_model if inconsistency_type == "style" else None,
        "verse_window": verse_window,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Local inconsistency types to create: name style. Default: style.",
    )
    parser.add_argument(
        "--rates",
        nargs="+",
    ...
    parser.add_argument(
        ...
        help=(
            "OpenAI model used for local style inconsistency clause rewrites. "
    ...
    parser.add_argument(
        ...
        type=int,
        default=12,
        help="Number of selected Chinese clauses to send per local rewrite request.",
    )
    parser.add_argument(
        "--name-bank-dir",
        type=Path,
        default=DEFAULT_BANK_DIR,
        help="Optional per-chapter name bank directory used only with --types name.",
    )
...
def main() -> int:
    ...
    for chapter in args.chapters:
        ...
        if not source_method_dir.exists():
            ...
            continue
        name_variants = (
            load_chapter_bank(chapter, args.name_bank_dir)
            if "name" in inconsistency_types
            else None
        )
