...
def bank_filename(chapter: Any) -> str:
    """Bank filename for a cell.

    Must match bank_path_for_chapter in create_awkward_style_variants.py or the
    variant script will not find what this writes.
    """
    if isinstance(chapter, int) or str(chapter).isdigit():
        return f"luke{chapter}_awkward_style_bank.json"
    return f"{str(chapter).replace('/', '_')}_awkward_style_bank.json"
...
def build_prompt(
    *,
    ...
    # See bank_filename: chapter may be an int (Luke) or a passage-dir name.
...
def main() -> int:
    ...
    try:
        ...
        for chapter in targets:
            output_path = args.output_dir / bank_filename(chapter)
