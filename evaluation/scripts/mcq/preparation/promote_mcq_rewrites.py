...
<name>.bak once before writing.
  ...
  # preview, no writes
  python evaluation/scripts/mcq/preparation/promote_mcq_rewrites.py --dry-run
  # apply (backs up each file to .bak)
  python evaluation/scripts/mcq/preparation/promote_mcq_rewrites.py
  # only the delivered clean dir (omission/0%), leave LLM condition dirs untouched
  python evaluation/scripts/mcq/preparation/promote_mcq_rewrites.py --clean-only
  # undo
  python evaluation/scripts/mcq/preparation/promote_mcq_rewrites.py --restore
"""
import argparse
import glob
import json
