...
open variants.
  ...
  python evaluation/scripts/mcq/legacy_luke/regen_mcq_tier01.py --provider ollama
  python evaluation/scripts/mcq/legacy_luke/regen_mcq_tier01.py --provider ollama --skip-promote --models "1.5b=qwen2.5:1.5b"
"""
...
def promote(root):
    here = Path(__file__).resolve().parents[3]           # evaluation/
    repo = here.parent
    print(">> rebuilding datasets/mcq/mcq_rewrites.json (scripts/mcq/preparation/build_rewrites_v2.py) ...")
    subprocess.run(
        [sys.executable, str(here / "scripts" / "mcq" / "preparation" / "build_rewrites_v2.py")],
        cwd=repo,
        check=True,
    )
