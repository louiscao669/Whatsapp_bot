...
PLATFORM_REPLACEMENTS = [
    ("from app.spa_views", "from backend.admin.spa_views"),
    ("from app.admin_nav", "from backend.admin.nav"),
    ("from app.utils.admin_formatters", "from backend.admin.utils.admin_formatters"),
    ("from app.utils.media_urls", "from backend.admin.utils.media_urls"),
    ("from app.services.workflow import AssignmentAssignError", "from backend.admin.services.workflow_bridge import AssignmentAssignError"),
    (
        "from app.services.workflow import (\n    AssignmentAssignError,\n    assign_qa_item_to_participant,\n    get_or_create_participant_session,\n)",
        "from backend.admin.services.workflow_bridge import (\n    AssignmentAssignError,\n    assign_qa_item_to_participant,\n    get_or_create_participant_session,\n)",
    ),
]
...
def fix_qa_review_service():
    path = REPO / "platform/backend/admin/services/qa_review_service.py"
    text = path.read_text()
    text = text.replace(
    ...
    path.write_text(text)
    print("updated platform/backend/admin/services/qa_review_service.py")
...
def fix_qa_keywords():
    ...
    text = text.replace(
        ...
        "from backend.admin.services.uw_qa_import_service import",
    )
