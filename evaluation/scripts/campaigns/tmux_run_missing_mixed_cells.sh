...
#   bash evaluation/scripts/campaigns/tmux_run_missing_mixed_cells.sh
#   MODELS="llama1b qwen1.5b" bash evaluation/scripts/campaigns/tmux_run_missing_mixed_cells.sh
#
# Then:
...
tmux send-keys -t "$SESSION" \
  "$KEEPAWAKE env PYTHONUNBUFFERED=1 MODELS='${MODELS}' bash evaluation/scripts/campaigns/run_missing_mixed_cells.sh 2>&1 | tee '${LOG}'; echo; echo '=== RUN FINISHED — log: ${LOG} ==='" C-m
