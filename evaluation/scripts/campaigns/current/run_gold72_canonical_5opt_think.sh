...
#   MCQ_CHOICE_LABELS=ABCDE    FIVE options: A-D plus ONE meta-option on E.
#
# Five, not six, and that is the entire difference from the 6opt_think arm. The second
# meta-option (F, 以上都不是) was measured and dropped on 2026-09-14: offered alongside E
# the choice between them tracked which LETTER each role sat on rather than what either
# label said, and no rewording fixed it -- relabelling both options and stating the
# divide as a numbered procedure moved the result by exactly zero. Removing F moved it
# from 50.0% to 62.5-87.5% on the same items (p = 0.031 in the best cell) while
# abstention held at 81-94%. E's own string and _meta_hint() are UNCHANGED: E already
# absorbs F's case, which is a bug only while F exists.
# See EXPERIMENT_META_OPTION_COLLAPSE_2026-09-14.md.
#
# NOTE: chance is 1/5 here and 1/6 in the 6opt_think arm. The two are NOT comparable,
# and neither is comparable to any 4-option run.
...
#
#   bash evaluation/scripts/campaigns/current/run_gold72_canonical_5opt_think.sh
...
# No MCQ_NOTA_LABEL: nota auto-disables below six labels, and _meta_hint() then emits
# the abstention clause alone -- exactly the prompt the five-option arm was validated on.
export QA_FORMATS=mcq
export NO_THINK=0
export OUT_ROOT=evaluation/outputs/tier1_bsb_unblinded_5opt_think
# 71 MCQ items, keys balanced 18/18/18/17 across A-D (rebalance_key_positions.py).
...
# The QA set must carry exactly the option count this arm answers with. Checked against
# MCQ_CHOICE_LABELS rather than a hard-coded 6, so the 5-option and 6-option arms share
# one guard: a six-option QA answered at ABCDE would silently drop F off the bottom, and
# a five-option QA answered at ABCDEF would offer an F that no item has.
want=${#MCQ_CHOICE_LABELS}
nbad=$(WANT="$want" python3 - <<'PY'
import json, os
d, want = os.environ["QA_DIR"], int(os.environ["WANT"])
bad = 0
for fn in sorted(os.listdir(d)):
    if not fn.endswith(".json"): continue
    for it in json.load(open(os.path.join(d, fn))):
        m = it.get("mcq")
        if m and len(m.get("mcq_options") or []) != want: bad += 1
print(bad)
PY
)
[ "$nbad" = "0" ] || { echo "$nbad item(s) in $QA_DIR do not have $want options" >&2; exit 1; }
