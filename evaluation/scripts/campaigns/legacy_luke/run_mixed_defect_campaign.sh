...
#
# USAGE (repo root):
#   bash evaluation/scripts/campaigns/legacy_luke/run_mixed_defect_campaign.sh generate
#   bash evaluation/scripts/campaigns/legacy_luke/run_mixed_defect_campaign.sh answer
...
gen_pair() {  # $1=first_defect $2=src_rate $3=second_name $4=generator $5...=extra
  ...
  run python3 "evaluation/scripts/variants/current/${GEN}" \
...
if [ "$STAGE" = "generate" ]; then
  ...
  echo "GENERATED. Next: bash evaluation/scripts/campaigns/legacy_luke/run_mixed_defect_campaign.sh answer"
  exit 0
fi
