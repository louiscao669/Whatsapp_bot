...
#
# Usage:
#   bash evaluation/scripts/run_missing_mixed_cells.sh          # qwen1.7b
#   MODELS="llama1b qwen1.5b" bash evaluation/scripts/run_missing_mixed_cells.sh
#   DRY_RUN=1 bash evaluation/scripts/run_missing_mixed_cells.sh  # preview
set -euo pipefail
cd "$(dirname "$0")/../.."

MODELS="${MODELS:-qwen1.7b}"
DRY_RUN="${DRY_RUN:-0}"
CAMPAIGN=evaluation/scripts/run_mixed_defect_campaign.sh
