...
def normalize_judgment(raw: dict) -> dict:
    ...
    # core_claim_found is listed in the judge's output schema but never defined
    # in the prompt, while the three score levels are specified precisely. The
    # model therefore fills it from its own reading and drifts toward "touched
    # the core topic", disagreeing with the score it ships alongside: on
    # judg9/llama321b, items 9, 23 and 25 came back 0.5 with core_claim_found
    # true, though the rubric reserves "contains the core claim" for 1.0.
    #
    # Recorded, not repaired. Overwriting it would hide the rate, and the rate is
    # what matters: QA_algorithm/ estimates IRT difficulty b_i, per-item
    # sensitivity s_item and Fisher-information selection from single-scored
    # items, so it needs to know which per-item fields are trustworthy. Judge
    # labels already flip at ~18% run to run (4.5% at temperature 0).
    core_claim_found = raw.get("core_claim_found")
    judge_inconsistent = (
        isinstance(core_claim_found, bool) and core_claim_found != (score == 1.0)
    )
    ...
    return {
        ...
        "core_claim_found": core_claim_found,
...
def create_text_response(
    ...
    *,
    ...
    """Single choke point for OpenAI requests in this module.

    ``temperature`` and ``seed`` are omitted from the request when None, which
    reproduces the historical behaviour exactly (API default temperature 1.0,
    no seed). Set temperature=0 to make judging and back-translation
    reproducible: at the default, the judge is itself a sampled draw, so the
    same answers can score differently on a rerun.
    """
    ...
    if hasattr(client, "responses"):
        # The Responses API rejects seed; only temperature applies here.
        return client.responses.create(model=model, input=input, **sampling)
...
def apply_reverse_glossary(text: str, reverse_glossary: Optional[Dict[str, str]]) -> str:
    """Rewrite target-language pseudonyms back to their English form.

    The standard answers are written with the English pseudonyms (Tadul, Divus).
    Back-translation, left to itself, transliterates the Chinese afresh -- 韦恩
    became "Wayne", 尼温 became "Niwen", 慕斯 became "Moose" -- so the judge
    compared two different naming systems and scored substantively correct
    answers as wrong. Applying the map in reverse first puts both sides in the
    same namespace.
    """
    if not reverse_glossary or not text:
        return text
    for chinese in sorted(reverse_glossary, key=len, reverse=True):
        text = text.replace(chinese, reverse_glossary[chinese])
    return text
...
def translate_open_answers_to_english(
    ...
    temperature: Optional[float] = None,
    seed: Optional[int] = None,
    reverse_glossary: Optional[Dict[str, str]] = None,
) -> None:
    if not rows:
        return
            ...
            "items": [
                {
                    ...
                    "generated_answer": apply_reverse_glossary(
                        normalize_chinese_placeholders_to_english(
    ...
    *,
    ...
    if open_rows:
        ...
        translate_open_answers_to_english(
            ...
            temperature=temperature,
            seed=seed,
            reverse_glossary=reverse_glossary,
        )
...
def score_items(
    ...
    *,
    ...
    for generated in generated_items:
        ...
        # Per-item generation effort from the answer model's own counters.
        # Carried into the scores file so effort sits alongside correctness for
        # the same item, which is what makes "did harder items cost more?"
        # answerable without re-joining two files.
        if "answer_effort" in generated:
            row["answer_effort"] = generated.get("answer_effort")
    ...
    if open_rows and not skip_llm:
        for batch in batched(open_rows, judge_batch_size):
            ...
            for row in batch:
                ...
                row["llm_judge_inconsistent"] = english_judgment.get(
                    "judge_inconsistent"
                )
...
def summarize(scored: List[dict]) -> dict:
    ...
    judged_items = [
        item for item in open_items if item.get("llm_core_claim_found") is not None
    ]
    judge_inconsistent = [
        item for item in judged_items if item.get("llm_judge_inconsistent")
    ]
    confidence_values = [
    ...
    return {
        ...
        # How often the judge's core_claim_found disagreed with its own score.
        # A diagnostic of judge self-consistency, not of answer quality: it does
        # not enter accuracy. Read it before using any per-item judge field for
        # IRT calibration or Fisher-information item selection.
        "judge_inconsistent_count": len(judge_inconsistent),
        "judge_inconsistent_rate": (
            len(judge_inconsistent) / len(judged_items) if judged_items else None
        ),
    }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=0.0,
        help=(
            "Sampling temperature for back-translation and the LLM judge. "
            "Defaults to 0 for reproducible scoring. Pass 1.0 to reproduce "
            "pre-2026-08-03 runs."
        ),
    )
    parser.add_argument("--seed", type=int, help="Sampling seed for OpenAI calls.")
