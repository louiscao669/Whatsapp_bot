...
Exit code is 0 only when nothing is FAIL.
  ...
  python human_pilot/verify_pilot_readiness.py
  python human_pilot/verify_pilot_readiness.py --participant-ids id0,id1
  python human_pilot/verify_pilot_readiness.py --participant-ids id0,id1 --fix
...
unless you pass ``--allow-schema-rewrite``. It also deliberately refuses to:
  ...
  * delete anything -- use ``human_pilot/pilot_wipe.py`` deliberately.
...
def check_participants(db, report, participants, language, tables):
    ...
    if planless:
        report.add(
            ...
            f"condition",
            "python human_pilot/build_experiment_plan.py --participant-ids <ids>",
        )
    elif partial:
        report.add(
            "participants: plan cells",
    ...
    if broken:
        report.add(
            ...
            f"python human_pilot/verify_experiment_delivery.py --language {language}",
        )
    elif any(
        db.scalar(
            select(func.count(ExperimentPlanCell.id)).where(
...
def check_hygiene(db, report, participants, tables):
    ...
    if prior_assignments or prior_responses:
        report.add(
            ...
            "python human_pilot/pilot_wipe.py  (deliberate; --fix will not delete)",
        )
    else:
    ...
    if needs_plan and participants:
        ...
        if not consented:
            ...
            result = subprocess.run(
                [sys.executable, str(REPO_ROOT / "human_pilot" / "build_experiment_plan.py"),
...
def main():
    ...
    from backend.shared.config import load_configurations  # noqa: E402
