...
Usage (from repo root; needs DATABASE_URL or .env):
  python human_pilot/export_pilot_metrics.py --stdout
  python human_pilot/export_pilot_metrics.py --out-dir evaluation/reports/pilot
  python human_pilot/export_pilot_metrics.py --format csv --out-dir /tmp/pilot
  python human_pilot/export_pilot_metrics.py --participant-ids id0,id1 --include-trials
  python human_pilot/export_pilot_metrics.py --database-url sqlite:///local.db --stdout
"""
...
def main():
    ...
    # reads the repo-root .env, so a bare `python human_pilot/...` run works.
