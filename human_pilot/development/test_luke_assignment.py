...
Usage (from repo root):
  python human_pilot/development/test_luke_assignment.py
  python human_pilot/development/test_luke_assignment.py --seed-only
  python human_pilot/development/test_luke_assignment.py --content-id 174314
  python human_pilot/development/test_luke_assignment.py --json-path "/path/to/uw-translation-questions-eng-luke.json"
  python human_pilot/development/test_luke_assignment.py --answer "The eyewitnesses were with the apostles from the beginning of Jesus ministry"

  # Use an existing QA row in Supabase (no UW JSON file):
  python human_pilot/development/test_luke_assignment.py --from-db --content-id 174345 --wa-id 15551234567 --name "Test User 3"
  python human_pilot/development/test_luke_assignment.py --from-db --passage-id uw-174345 --assign --answer "In Rome"
  python human_pilot/development/test_luke_assignment.py --from-db --qa-item-id f787affd-26a9-4b3e-b3fa-46581965a21d --assign
"""
...
use_message_bot()
# The UW compatibility parser is owned by the platform backend. Append rather
# than prepend so the message-bot's `app` package remains the active `app`.
sys.path.append(str(REPO_ROOT / "platform"))

from sqlalchemy import select

from app import create_app
from eten_shared.database import get_session_factory
from eten_shared.models import QAItem, SessionState
...
from uw_qa_content import find_uw_entry, qa_item_payload_from_uw_entry
...
def assign_question(db, wa_id, display_name, language, qa_item=None):
    ...
    print(f"  QA item detail: /qa-items/{prompt.qa_item_id}")
    print("  QA items list:  /qa-items")
