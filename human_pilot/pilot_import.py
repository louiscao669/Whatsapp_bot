...
Usage:
  ...
  python human_pilot/pilot_import.py --dry-run
...
# The human study uses the BSB-based Gold-72 evaluation corpus. Keep this
# explicit so a normal import cannot silently fall back to the older Tier-1
# experiment outputs.
TIER1_ROOT = "tier1_bsb"
TIER1_GROUP_COUNT = 8
...
#
# Produced by `evaluation/scripts/analysis/current/report_tier1_family_split.py --emit-sensitivity`, which
...
#
# NOT the IRT s_i from QA_algorithm/scripts/modeling/fit_item_sensitivity.py: that is partially
...
# Tier-1 BSB translation already replaces the English pseudonyms with their
# glossary-controlled Chinese forms during decanonicalization. Unlike the old
# Luke pipeline, it does not need a second ``*_pseudonymized`` remap pass.
_UNRESOLVED_STYLE_PLACEHOLDER = re.compile(
    r"\b(?:PERSON|PLACE|PEOPLE|DEITY|LOCATION)_[A-Za-z0-9]+\b"
    r"|人物[甲乙丙丁戊己庚辛壬癸]|地点[甲乙丙丁戊己庚辛壬癸]",
    re.IGNORECASE,
)


def _pick_bsb_participant_file(d: Path, decanon: str, raw: str) -> Path:
    """Return a participant-ready BSB artifact and reject leaked placeholders."""
    path = d / decanon
    if not path.exists():
        path = d / raw
    if not path.exists():
        raise FileNotFoundError(f"missing participant-facing BSB artifact in {d}")
    match = _UNRESOLVED_STYLE_PLACEHOLDER.search(path.read_text(encoding="utf-8"))
    if match:
        raise ValueError(f"unresolved style placeholder {match.group(0)!r} in {path}")
    return path
...
def load_tier1_qa(eval_root: Path, passage_id: str) -> dict:
    ...
    records = json.loads(_pick_bsb_participant_file(
        d, "qa_target_decanonicalized.json", "qa_target.json"
...
def load_tier1_passage(eval_root: Path, passage_id: str, rel: str):
    ...
    if d is None:
        return None
    return _pick_bsb_participant_file(
        d, "passage_target_decanonicalized.txt", "passage_target.txt"
