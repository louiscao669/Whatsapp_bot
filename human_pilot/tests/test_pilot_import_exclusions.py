...
def load_planning_helpers():
    ...
    src = (REPO_ROOT / "human_pilot" / "pilot_import.py").read_text(encoding="utf-8")
    tree = ast.parse(src)
...
def test_build_plan_pool_guard():
    ...
    check("BSB Gold-72 produces 70 unique deliverable QA windows",
          len(qa_rows) == len(window_rows) == 70)
    ...
    check("no two imported questions share an exact window", len(identities) == 70)
    check("seven variants x ten passages are staged", len(passage_rows) == 70)
    check("all required BSB variants, including WBW, are present", not missing)
    check("summary records the 2 removed window collisions",
          "removed 2 extra question" in "\n".join(summary))


def test_bsb_participant_files_reject_raw_placeholders():
    """Mechanical style labels must never reach a participant-facing import."""
    from tempfile import TemporaryDirectory
    from pilot_import import _pick_bsb_participant_file

    with TemporaryDirectory() as tmp:
        root = Path(tmp)
        artifact = root / "passage_target_decanonicalized.txt"
        artifact.write_text("有一个名叫人物甲的人。", encoding="utf-8")
        try:
            _pick_bsb_participant_file(
                root, "passage_target_decanonicalized.txt", "passage_target.txt"
            )
        except ValueError as exc:
            check("raw style placeholder blocks the import", "人物甲" in str(exc))
        else:
            check("raw style placeholder blocks the import", False)
...
def main():
    for label, fn in [("allowed_forms", test_allowed_forms),
                      ...
                      ("BSB placeholder guard", test_bsb_participant_files_reject_raw_placeholders),
