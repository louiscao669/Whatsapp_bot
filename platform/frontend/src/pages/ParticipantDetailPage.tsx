...
export function ParticipantDetailPage() {
  ...
  async function handleLanguageSubmit(event: FormEvent) {
    ...
    try {
      ...
      setDetail({
        ...
        assigned_questions: result.assigned_questions,
  ...
  function handleAssigned(successMessage: string) {
    ...
    fetchParticipantDetail(participantId)
      .then(setDetail)
      .catch((err) =>
        setError(err instanceof ApiError ? err.message : 'Could not refresh participant'),
      )
  }
  ...
  const assignedQuestions = detail.assigned_questions ?? []
  ...
  return (
    <section className="panel detail-page">
      ...
      <ParticipantAssignmentPanel participantId={participant.id} onAssigned={handleAssigned} />

      <section className="detail-card">
        <h3>Questions assigned ({assignedQuestions.length})</h3>
