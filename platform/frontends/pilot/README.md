...
```bash
python human_pilot/pilot_import.py   # BSB passages + QA items
python human_pilot/build_experiment_plan.py --participant-ids id0,id1
python human_pilot/verify_experiment_delivery.py --language zh
```
...
```bash
python human_pilot/export_pilot_metrics.py --out-dir evaluation/reports/pilot --include-trials
```
...
platform/frontends/pilot/
  ...
  tests/timing.test.mjs `node --test tests/timing.test.mjs`
platform/backend/pilot/
